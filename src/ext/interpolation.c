  ...
  int** pmeta)
{
  ...
  *pindex = NULL;
  ...
  blob = aligned_alloc(SSE_ALIGNMENT, totalreq);
  if (!blob)
  {
    return 1;
  }
...
#undef RESAMPLING_BORDER_MODE BORDER_REPLICATE
#undef INTERPOLATION_BORDER_MODE BORDER_MIRROR
#undef SSE_ALIGNMENT
#undef MAX_HALF_FILTER_WIDTH
#undef DEBUG_RESAMPLING_TIMING
